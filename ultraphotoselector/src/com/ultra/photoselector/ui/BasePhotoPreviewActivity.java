...
public class BasePhotoPreviewActivity extends Activity implements OnPageChangeListener, OnClickListener {
	...
	protected boolean isReceived = false;//接收到的图片
	protected boolean isReceiveds = false;//接收到多张图片
	protected String receivedImgUrl = null;
	...
	private PagerAdapter mPagerAdapter = new PagerAdapter() {
		...
		public View instantiateItem(final ViewGroup container, final int position) {
			...
			{
				if(isReceiveds){
					photoPreview.loadImage(photos.get(position),0);
				} else {
					photoPreview.loadImage(receivedImgUrl);
				}
				photoPreview.setOnClickListener(photoItemClickListener);
			}
			else {
				photoPreview.loadLocalImage(photos.get(position));
				photoPreview.setOnClickListener(photoItemClickListener);
			}
